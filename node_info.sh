...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo
		echo -e "${C_LGn}Functionality${RES}: the script shows information about an Akash node"
		echo
		...
		echo -e "https://github.com/SecorD0/Akash/blob/main/node_info.sh - script URL"
...
token_name="akt"
node_dir="$HOME/.akash/"
wallet_name="$akash_wallet_name"
wallet_address="$akash_wallet_address"
wallet_address_variable="akash_wallet_address"
validator_address="$akash_validator_address"
validator_address_variable="akash_validator_address"
...
main() {
	...
	# Send Pull request with new texts to add a language - https://github.com/SecorD0/Akash/blob/main/node_info.sh
	#elif [ "$language" = ".." ]; then
	else
	...
	local status=`$daemon status --node "$local_rpc" 2>/dev/null`
